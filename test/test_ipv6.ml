...
let udp_message = Bytes.of_string "hello on UDP over IPv6"
...
let check_for_one_udp_packet on_received_one ~src ~dst buf =
  ...
  | Ok (_, payload) ->
    ...
    Alcotest.(check bytes) "payload is correct" udp_message payload
...
let dad_na_is_sent () =
  ...
  let nd buf =
    ...
    Ipaddr_bytes.V6.write_bytes_exn Ipaddr.V6.unspecified (Bytes.shift buf 8);
    Ipaddr_bytes.V6.write_bytes_exn (Ipaddr.V6.Prefix.network_address solicited_node_prefix address) (Cstruct.shift buf 24);
    ...
    Ipaddr_bytes.V6.write_bytes_exn address (Cstruct.shift icmpbuf 8);
  ...
  and is_na buf =
    ...
    Ipaddr.V6.compare
      (Ipaddr_bytes.V6.of_bytes_exn (Cstruct.shift buf 8))
      address = 0 &&
    Ipaddr.V6.compare
      (Ipaddr_bytes.V6.of_bytes_exn (Cstruct.shift buf 24))
    ...
    Ipaddr.V6.compare
      (Ipaddr_bytes.V6.of_bytes_exn (Cstruct.shift icmpbuf 8))
      address = 0
  in
  Lwt.pick [
    listen stack;
...
let dad_na_is_received () =
  ...
  let is_ns buf =
    ...
    if
      ...
      Ipv6_wire.get_code icmpbuf = 0
    then
      Some (Ipaddr_bytes.V6.of_bytes_exn (Cstruct.shift icmpbuf 8))
    else
      None
  in
  let na addr buf =
    ...
    Ipaddr_bytes.V6.write_bytes_exn addr (Cstruct.shift buf 8);
    Ipaddr_bytes.V6.write_bytes_exn Ipaddr.V6.link_nodes (Cstruct.shift buf 24);
    Ipv6_wire.set_hlim buf 255;
    Ipv6_wire.set_nhdr buf (Ipv6_wire.protocol_to_int `ICMP);
    let hdr, icmpbuf = Cstruct.split buf Ipv6_wire.sizeof_ipv6 in
    ...
    Ipaddr_bytes.V6.write_bytes_exn addr (Cstruct.shift icmpbuf 8);
    let optbuf = Cstruct.shift icmpbuf Ipv6_wire.Na.sizeof_na in
    Ipv6_wire.set_ty optbuf 2;
    Ipv6_wire.Llopt.set_len optbuf 1;
    Macaddr_bytes.write_bytes_exn mac (Cstruct.shift optbuf 2);
    Ipv6_wire.Icmpv6.set_checksum icmpbuf 0;
    Ipv6_wire.Icmpv6.set_checksum icmpbuf @@ Ndpv6.checksum hdr [icmpbuf];
    na_size
  in
